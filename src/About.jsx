...
export default function WorkoutRoutines() {
  ...
  return (
    <div
      className="relative w-full min-h-screen bg-black text-white font-bold text-lg"
      data-aos="fade-up"
      id="about"
    >
      {/* Background Image */}
      ...
      {/* Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-20 flex flex-col lg:flex-row gap-10">
        {/* Left Heading */}
        <div className="lg:w-1/3">
          <h1 className="text-yellow-300 text-5xl sm:text-6xl font-anton leading-tight texty">
            OUR <br /> WORKOUT <br /> ROUTINES
          </h1>
        </div>
        ...
        <div className="lg:w-2/3 space-y-14">
          {[
            ...
            {
              title: "TRAIN LIKE A CHAMP",
              description:
                "Take your training to the next level with high-intensity routines that challenge your strength, agility, and mindset. Designed for athletes and goal-getters who want to push limits and see serious gains.",
            },
            {
              title: "MAINTAIN THE GAIN",
              description:
                "Stay in peak form with sustainable routines that help you preserve your progress. Balance strength, cardio, and flexibility while preventing burnout or plateaus.",
            },
          ].map((block, index) => (
            <div key={index} className="flex flex-col sm:flex-row gap-4">
              <h2 className="text-2xl sm:text-3xl font-anton min-w-[150px] texty">
                {block.title}
              </h2>
              <p className="text-white max-w-xl">{block.description}</p>
            </div>
          ))}

          {/* Footer */}
          <p className="text-white text-2xl sm:text-3xl max-w-xl pt-4">
            Every body is different, so we make sure you can choose a plan that
            works best for you.
          </p>
        </div>
      </div>
